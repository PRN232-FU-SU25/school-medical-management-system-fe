...
import { useDeleteContractType } from '@/queries/contract.query';
...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const { mutateAsync: deleteContractType } = useDeleteContractType();

  const onConfirm = async () => {
    const res = await deleteContractType(data.id);
    ...
    <>
      ...
      >
        <DialogContent className="max-h-[90%] max-w-[80%] overflow-y-auto bg-white 2xl:max-w-[70%]">
          ...
          <ContractTypeDialog type="view" contractType={data} />
        </DialogContent>
      </Dialog>
